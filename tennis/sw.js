/* 速報 — Service Worker
 *
 * スコープは /tennis/ 配下のみ。サイト本体（政策分析ページ）や
 * 他のウィジェットには一切干渉しない。
 *
 * 方針:
 *   - 器（HTML・アイコン）だけを事前キャッシュし、圏外でも画面が開くようにする。
 *     スコアの取得は常に網へ出るので、キャッシュには一切載せない。
 *   - HTML は network-first。更新版があれば取りに行き、無ければ手元の版を返す。
 */

var VERSION = "v1";
var CACHE = "tennis-shell-" + VERSION;
  ...
  event.waitUntil(
    caches.keys().then(function(keys){
      return Promise.all(keys.map(function(key){
        if(key.indexOf("tennis-shell-") === 0 && key !== CACHE) return caches.delete(key);
      }));
    }).then(function(){
